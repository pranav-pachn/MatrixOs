...
from app.adapters.hospital.events import get_events
...
class HospitalAdapter(BaseAdapter):
    ...
    def metrics(self) -> List[AdapterMetric]:
        return [
            AdapterMetric(key="patientWaitTime", value=22.5, unit="mins", label="Patient Wait Time"),
            AdapterMetric(key="bedUtilization", value=92.0, unit="%", label="Bed Utilization"),
    ...
    def evaluate_invariants(self, scenario: Scenario, actions: List[RecoveryAction]) -> List[InvariantResult]:
        ...
        if validation.valid:
            return [InvariantResult(status=ValidationStatus.PASS, constraint="Hospital ER Rules", reason="All constraints passed.")]
        else:
            return [InvariantResult(status=ValidationStatus.REJECTED, constraint="Hospital Rules", reason=err) for err in validation.errors]
